...
def detect_market_regime(
    ...
    is_trending = adx_strength > 0.35 and trend_pressure > 0.25
    is_ranging = adx_strength < 0.20 and low_efficiency
    ...
    if phase in {"consolidation", "range_bound"} and vol_regime == "expanding" and adx_strength > 0.30:
        phase = "trending"
...
def _compute_adx_proxy(closes: list[float], metrics: MarketMetrics) -> float:
    ...
    spread_strength = min(spread_pct / 0.5, 1.0)  # Normalize: 0.5% spread = max strength
