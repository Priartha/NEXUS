"""
Test optimized signals with all improvements on 30-day data.
"""
...
def generate_signals(candles, sl_mult=2.0, adx=True, limit=True, min_conf=0.55, cooldown=12):
    ...
    print(f"  Generating (SL={sl_mult}x, ADX={adx}, Limit={limit}, Conf={min_conf})...", end=" ", flush=True)
...
def main():
    print("=" * 80)
    print("  OPTIMIZED SIGNALS TEST (30-DAY DATA)")
    print("=" * 80)
        ...
        for adx in [True, False]:
            for limit in [True, False]:
                configs.append({
                    ...
                    "tp1": 1.0,
                    "tp2": 2.0,
                    "name": f"SL={sl}x ADX={'Y' if adx else 'N'} LMT={'Y' if limit else 'N'}",
                })
    
    # Additional configs with different exits
    configs.extend([
        {"sl_mult": 2.0, "adx": True, "limit": True, "hold": 12, "be": 0.5, "tp1": 1.0, "tp2": 1.5, "name": "SL=2.0x Quick Exit"},
        {"sl_mult": 2.5, "adx": True, "limit": True, "hold": 50, "be": 1.0, "tp1": 1.0, "tp2": 3.0, "name": "SL=2.5x Wide Exit"},
        {"sl_mult": 2.0, "adx": True, "limit": True, "hold": 25, "be": 1.0, "tp1": 1.0, "tp2": 2.0, "name": "SL=2.0x BE@1R"},
    ])
    ...
    print("  SUMMARY")
    print("=" * 100)
    print(f"  {'#':<3} {'Name':<30} {'Signals':<8} {'Trades':<7} {'Win%':<7} {'PF':<7} {'P&L%':<8} {'DD%':<7} {'Sharpe':<7}")
    print("-" * 100)
    ...
    # Save
    output_path = Path(__file__).parent.parent / "optimized_30d_results.json"
