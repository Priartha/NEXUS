"""Compare v3 vs v5 engines on 30 days of data."""
...
from backend.analysis.signals_v5 import detect_trade_signals as detect_v5
...
async def main():
    ...
    print("=" * 80)
    print("NEXUS v3 vs v5 - 30 DAY BACKTEST")
    print("=" * 80)
    ...
    print("\n[2/2] v5 (range-optimized)...")
    start = time.time()
    r = backtest_engine("v5_range", detect_v5, candles, step=10)
    print(f"  {time.time()-start:.1f}s: {r['signals']} signals, WR: {r['win_rate']:.1f}%, PnL: ${r['total_pnl']:.2f}")
    results.append(r)
