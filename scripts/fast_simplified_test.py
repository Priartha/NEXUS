"""
Fast test of simplified signals on 7-day sample.
"""
...
def generate_signals_fast(candles, use_htf=True, min_conf=0.45):
    """Fast signal generation."""
    print(f"  Generating signals (HTF={use_htf}, min_conf={min_conf})...", end=" ", flush=True)
    
    all_signals = []
    lookback = 80
    min_candles = max(lookback, 50)
    
    # Pre-compute some values to speed up
    for i in range(min_candles, len(candles)):
        window = candles[:i + 1]
        recent = window[-lookback:]
        current = candles[i]
        
        # Simplified analysis (skip some heavy computations)
        ...
        # Update patterns
        for c in recent[-20:]:  # Only last 20 candles
...
def run_bt(candles, signals, pos_sz=0.02, hold=25, be=0.75, tp1=1.0, tp2=2.0):
    """Run backtest."""
...
def main():
    print("=" * 80)
    print("  FAST SIMPLIFIED SIGNALS TEST (7-DAY SAMPLE)")
    print("=" * 80)
    ...
    # Test key configs
    configs = [
        {"use_htf": True, "min_conf": 0.45, "hold": 25, "be": 0.75, "tp1": 1.0, "tp2": 2.0, "name": "SIMP+HTF Default"},
        {"use_htf": True, "min_conf": 0.40, "hold": 25, "be": 0.75, "tp1": 1.0, "tp2": 2.0, "name": "SIMP+HTF Relaxed"},
        {"use_htf": False, "min_conf": 0.45, "hold": 25, "be": 0.75, "tp1": 1.0, "tp2": 2.0, "name": "SIMP No HTF"},
        {"use_htf": True, "min_conf": 0.45, "hold": 12, "be": 0.5, "tp1": 1.0, "tp2": 1.5, "name": "SIMP Quick"},
        {"use_htf": True, "min_conf": 0.50, "hold": 25, "be": 1.0, "tp1": 1.0, "tp2": 2.0, "name": "SIMP Strict"},
    ]
    ...
    # Save
    output_path = Path(__file__).parent.parent / "simplified_fast_results.json"
