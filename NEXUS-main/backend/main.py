...
import random
import math
from backend.models.types import Candle as CandleType
...
live_refresh_task: asyncio.Task | None = None
...
async def lifespan(app: FastAPI):
    ...
    # ── Synthetic candle generator (fallback when APIs unreachable) ──────
    def _generate_synthetic_candles(symbol: str, timeframe: str, count: int, base_price: float = 60000.0) -> list[CandleType]:
        ms_map = {"1m": 60000, "5m": 300000, "15m": 900000, "1h": 3600000, "4h": 14400000}
        ms = ms_map.get(timeframe, 300000)
        now = int(time.time() * 1000)
        start_ms = now - count * ms
        price = base_price
        candles: list[CandleType] = []
        for i in range(count):
            t = start_ms + i * ms
            drift = random.gauss(0, 0.0008)
            vol = base_price * 0.005 * (0.5 + random.random())
            o = price
            c = o * (1 + drift + random.gauss(0, 0.002))
            h = max(o, c) * (1 + random.random() * 0.003)
            l = min(o, c) * (1 - random.random() * 0.003)
            v = random.uniform(100, 5000) * (base_price / 60000)
            candles.append(CandleType(timestamp=t, open=round(o, 2), high=round(h, 2), low=round(l, 2), close=round(c, 2), volume=round(v, 2), is_closed=i < count - 1))
            price = c
        return candles
    ...
    for tf, store in stores.items():
        try:
            ...
            logger.warning(f"Failed to seed {tf}: {e} — generating synthetic data")
            candles = _generate_synthetic_candles(
                symbol=settings.symbol,
                timeframe=tf,
                count=settings.history_seed_candles,
                base_price=60000.0,
            )
            now_ms = candles[-1].timestamp if candles else None
            store.seed(candles, now_ms=now_ms)
            logger.info(f"Seeded {len(candles)} synthetic candles for {tf}")
    ...
    live_refresh_task = asyncio.create_task(live_refresh_loop())
    ...
    self_heal.register("live_refresh", live_refresh_task, lambda: asyncio.create_task(live_refresh_loop()))
    ...
    try:
        ...
        for task in (stream_task, sentiment_task, ai_ict_task, model_monitor_task, db_backup_task, futures_task, me_task, auto_research_task, xgboost_task, hmm_task, transformer_task, rl_task, nlp_task, onchain_task, cross_exchange_task, pipeline_task, hmm_predict_task, live_refresh_task):
            try:
                await task
...
async def hmm_predict_loop() -> None:
    """Periodically call HMM predict to populate recent_regimes history (5s for live signals)."""
    from backend.utils.self_heal import self_heal
    from backend.utils.panel_freshness import panel_freshness
...
async def live_refresh_loop() -> None:
    """Continuous live refresh — runs scalp + paper trading every 2s on every pipeline."""
    await asyncio.sleep(2)
    while True:
        try:
            for timeframe_scope, store in stores.items():
                pipeline = pipelines[timeframe_scope]
                live_result = await pipeline.live_resolve_async(store)
                if live_result:
                    await manager.broadcast(
                        {"update_type": "live", **live_result},
                        timeframe=timeframe_scope,
                    )
        except asyncio.CancelledError:
            raise
        except Exception:
            logger.exception("live refresh loop failed")
        await asyncio.sleep(2)
    ...
    try:
        ...
        try:
            ...
            for table_name, count in raw.get("tables", {}).items():
                if table_name not in tables_with_ts:
                    continue
                total_records += count
                oldest = None
                newest = None
