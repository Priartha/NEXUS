...
async def publish_tick(
    ...
    for timeframe, store in stores.items():
        ...
        if candle_closed:
            ...
            _t = asyncio.create_task(manager.broadcast(result, timeframe=timeframe))
            _background_tasks.add(_t); _t.add_done_callback(_background_tasks.discard)
            timeframe_updates.append(_t)
        elif store.live_candle is not None:
            pipeline = pipelines[timeframe]
            # Broadcast candle immediately for price updates — don't wait for analysis
            ...
            _background_tasks.add(_t); _t.add_done_callback(_background_tasks.discard)
            timeframe_updates.append(_t)
            # Fire-and-forget: scalp analysis + paper trading runs in background
            _t2 = asyncio.create_task(_publish_live_analysis(pipeline, store, manager, timeframe))
