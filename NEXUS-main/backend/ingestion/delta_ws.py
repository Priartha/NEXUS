...
_background_tasks_delta: set[asyncio.Task] = set()
...
async def _publish_live_analysis(
    ...
    """Fire-and-forget: run scalp + paper trading, broadcast full live payload."""
    try:
        live_result = await pipeline.live_resolve_async(store)
        if live_result:
            live_result.pop("candle", None)  # candle already broadcast by publish_tick, avoid stale overwrite
            await manager.broadcast(
                {
                    "update_type": "live",
                    "symbol": store.symbol,
    ...
    while True:
        try:
            ...
            async with websockets.connect(
                ...
                while True:
                    try:
                        raw = await asyncio.wait_for(websocket.recv(), timeout=max_silence_seconds)
                    except asyncio.TimeoutError:
                        logger.warning("No Delta messages for %ss, reconnecting", max_silence_seconds)
                        break
                    last_message_time = time.monotonic()
                    ...
                    for timeframe, store in stores.items():
                        ...
                        if candle_closed:
                            ...
                            _background_tasks_delta.add(_t); _t.add_done_callback(_background_tasks_delta.discard)
                            timeframe_updates.append(_t)
                            ...
                            _t2 = asyncio.create_task(
                                _publish_live_analysis(
                                    pipeline, store, manager, timeframe
                                )
                            )
                            _background_tasks_delta.add(_t2); _t2.add_done_callback(_background_tasks_delta.discard)
