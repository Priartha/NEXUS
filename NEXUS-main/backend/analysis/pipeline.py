...
class AnalysisPipeline:
    ...
    def live_resolve(self, store: CandleStore) -> dict:
        """Live analysis on every tick — scalp engine + paper trading with live candle.

        Uses cached pipeline state (regime, metrics, FVGs, OBs, etc.) and runs
        only the scalp engine + paper trading on the latest candles (including
        the in-progress live candle). Returns a lightweight payload with fresh
        signals and trade events. Does NOT hold the full pipeline lock.
        """
        closed = store.get_closed_candles()
        live = store.live_candle
        if not closed or len(closed) < 20:
            return {}

        candles_for_scalp = list(closed)
        if live:
            candles_for_scalp.append(live)

        # Snapshot shared state atomically to avoid race with run_async (thread pool)
        metrics = self.metrics
        regime = self.regime
        if metrics is None or regime is None:
            return {}
        fvgs = list(self.fvgs)
        order_blocks = list(self.order_blocks)
        swings = list(self.swings)
        hmm_regime = self.hmm_regime
        liquidity_events = list(self.liquidity_events)
        futures_context = self.futures_context
        projection = self.projection

        try:
            ctx = self.scalp_engine.compute(
                candles=candles_for_scalp,
                metrics=metrics,
                fvgs=fvgs,
        ...
        if ctx.signals:
            self._last_scalp_context = ctx
        self.scalp_context = ctx

        display_ctx = ctx
        if not display_ctx.signals and self._last_scalp_context is not None:
        ...
        result = {
            ...
            "symbol": store.symbol,
            "timeframe": store.timeframe,
            "candle": to_wire(live) if live else None,
            "scalp": to_wire(display_ctx) if display_ctx else None,
            "scalp_risk": self.scalp_risk.get_risk_summary(),
            "regime": to_wire(regime),
            "metrics": self._metrics_payload(),
            "projection": to_wire(projection),
        }

        # Convert scalp signals to TradeSignals for paper trading
        signals: list[TradeSignal] = []
        if display_ctx and display_ctx.signals:
            for ss in display_ctx.signals[:1]:
                side = "buy" if "LONG" in ss.signal_type else "sell"
                t_sig = TradeSignal(
                    id=ss.id,
                    timestamp=ss.timestamp,
                    side=side,
                    entry=ss.entry if ss.entry else round((ss.entry_zone_low + ss.entry_zone_high) / 2, 2),
                    stop_loss=ss.sl_level,
                    exit_price=ss.exit_price,
                    risk_reward=round(abs(ss.exit_price - ss.entry) / abs(ss.entry - ss.sl_level), 2) if ss.entry and ss.sl_level and abs(ss.entry - ss.sl_level) > 0 else ss.risk_reward,
                    confidence=ss.score,
                    reason=ss.reason,
                ...
                signals.append(t_sig)

        result["signals"] = to_wire(signals)

        # Paper trading evaluation on every tick
        if self._paper_trading and live:
            from backend.storage import repository as repo
            try:
                events = self._paper_trading.evaluate_signals(
                    signals, live, symbol=store.symbol, timeframe=store.timeframe,
                ...
                for ev in events:
                    if ev["type"] == "trade_opened":
                        ...
                        if hasattr(self_optimizer, 'optimize_on_close'):
                            self_optimizer.optimize_on_close(trade_data)
                    if ev["type"] == "trade_blocked":
                        self._on_alert(ev)
                result["paper_trading"] = repo.get_paper_trade_stats()
            except Exception as exc:
                logger.warning("Paper trading eval failed in live_resolve: %s", exc)
    ...
    async def live_resolve_async(self, store: CandleStore) -> dict:
        """Live scalp + paper trading on every tick — runs in the event loop (no thread pool, no race with run_async)."""
        return self.live_resolve(store)
    ...
    def _serialize(
        ...
        if closed_candles and len(closed_candles) >= 20:
            candles_for_scalp = list(closed_candles)
            live = store.live_candle
            if live:
                candles_for_scalp.append(live)
        ...
        if display_ctx and display_ctx.signals:
            for ss in display_ctx.signals:
                ...
                t_sig = TradeSignal(
                    ...
                    liquidity_score=0.5,
                    bias_score=0.5,
                    expected_move=abs(ss.exit_price - ss.entry),
                    win_probability=round(min(ss.risk_reward / (ss.risk_reward + 1), 0.85), 3),
                    kelly_fraction=round(max(0, (ss.score * ss.risk_reward - (1 - ss.score)) / max(ss.risk_reward, 0.01)) * 0.5, 4),
