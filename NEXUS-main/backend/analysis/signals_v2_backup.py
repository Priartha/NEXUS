...
def _detect_pullback(
    ...
    Bullish pullback: price dips toward EMA20/EMA50 in uptrend, RSI 35-55
    Bearish pullback: price rallies toward EMA20/EMA50 in downtrend, RSI 45-65
    """
    if len(candles) < 5:
    ...
    if direction == "buy":
        # Price should be above EMAs but pulling back
        if last.close < ema20:
            return False, "below_ema20"

        # Pullback to EMA20 zone (within 0.3%)
        ema20_distance = abs(last.close - ema20) / ema20
        if ema20_distance > 0.005:
            return False, "not_near_ema20"

        # RSI should be cooling off but not oversold
        if rsi < 35 or rsi > 65:
            return False, f"rsi {rsi:.1f} out of range"

        # Price should have been higher recently (pullback, not breakdown)
        if last.close > prev.close and prev.close > prev2.close:
            return False, "no_pullback_continuing_up"

        # Check for pullback confirmation (wick rejection or reversal candle)
        body = last.close - last.open
        lower_wick = min(last.open, last.close) - last.low
        if lower_wick > abs(body) * 0.5:
            return True, "pullback_to_ema20_with_rejection"

        # Or just a small body near EMA
        if abs(body) / (last.high - last.low + 1e-9) < 0.4:
            return True, "pullback_to_ema20_consolidation"

        return False, "no_pullback_signal"

    else:  # sell
        if last.close > ema20:
            return False, "above_ema20"
...
def _check_trend_structure(
    ...
    Returns (is_valid, reason, strength 0-1)
    """
    if len(swings) < 4:
        return False, "insufficient_swings", 0.0
    ...
    if direction == "buy":
        ...
        return False, f"weak_structure HL={hl_ratio:.0%}", hl_ratio

    else:
        if len(highs) < 2:
            return False, "no_highs", 0.0
        ...
        return False, f"weak_structure LH={lh_ratio:.0%}", lh_ratio
...
def detect_trade_signals(
    ...
    ema_aligned_bullish = ema9 > ema20 > ema50
    ema_aligned_bearish = ema9 < ema20 < ema50
    ...
    for direction in ["buy", "sell"]:
        # 1. TREND FILTER: Must have clear trend
        if direction == "buy" and not ema_aligned_bullish:
            continue
        if direction == "sell" and not ema_aligned_bearish:
            continue
        ...
        # Stop below recent swing low (buy) or above swing high (sell)
        # Use tighter stops: ATR-based, not pullback-based
        if direction == "buy":
            # Find nearest support below
            recent_lows = sorted([c.low for c in ordered[-20:]])
            support = recent_lows[2] if len(recent_lows) > 2 else ema50
            stop = min(support, ema50) - atr14 * 0.2
            target = entry + (entry - stop) * reward_multiple
        else:
            recent_highs = sorted([c.high for c in ordered[-20:]], reverse=True)
            resistance = recent_highs[2] if len(recent_highs) > 2 else ema50
            stop = max(resistance, ema50) + atr14 * 0.2
            target = entry - (stop - entry) * reward_multiple

        risk = abs(entry - stop)
        if risk < atr14 * 0.3 or risk > atr14 * 2.0:
