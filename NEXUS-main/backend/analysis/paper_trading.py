...
class PaperTradingEngine:
    ...
    def evaluate_signals(
        ...
        trade = {
            ...
            "original_tp": best.exit_price,
            "original_score": best.confidence,
    ...
    def _net_pnl(self, side: str, entry: float, exit_price: float, qty: float, trade: dict, candle: Candle) -> tuple[float, float, float, float]:
        ...
        bars_held = trade.get("bars_held", 1)
        funding_cost = self.funding_rate_per_8h * entry * qty * max(bars_held, 1)
        ...
        for trade in open_trades:
            ...
            if side == "buy":
                ...
                if self.trailing_atr_multiplier > 0 and highest > entry:
                    risk = abs(entry - trade.get("initial_stop", sl))
                    if risk > 0:
                        profit_r = (highest - entry) / risk
                        ...
                        if profit_r >= 1.5:
                            trailing_stop = highest - risk * self.trailing_atr_multiplier
                    ...
                    if risk > 0:
                        ...
                        if profit_r >= self.breakeven_at_r:
                            ...
                            if new_sl < sl:
                                ...
                                events.append({"type": "trailing_stop_breakeven", "trade_id": trade["id"]})
                        if profit_r >= 1.5:
                            trailing_stop = lowest + risk * self.trailing_atr_multiplier
    ...
    def _current_balance(self) -> float:
        ...
        open_trades = repo.get_paper_trades(status="open")
        unrealized_pnl = sum(
            (t.get("highest_price", t["entry_price"]) - t["entry_price"]) * t["quantity"]
            if t["side"] == "buy"
            else (t["entry_price"] - t.get("lowest_price", t["entry_price"])) * t["quantity"]
            for t in open_trades
        )
        return self.initial_balance + closed_pnl + unrealized_pnl
