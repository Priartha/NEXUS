...
class RLSizingAgent:
        ...
        if self._is_trained and self._policy is not None:
            try:
                ...
                with torch.no_grad():
                    ...
                    self._memory.append((state, float(output[0]), float(output[1])))
            except Exception:
    ...
    def train(self) -> dict:
        ...
        if len(self._episode_rewards) >= 50 and len(self._memory) >= self.batch_size:
            if self._build_policy():
                import torch
                import torch.optim as optim

                self._policy.train()
                optimizer = optim.Adam(self._policy.parameters(), lr=self.lr)

                batch = random.sample(list(self._memory), self.batch_size)
                states = torch.FloatTensor(np.array([b[0] for b in batch]))
                actions = torch.FloatTensor(np.array([[b[1], b[2]] for b in batch]))

                rewards = np.array(list(self._episode_rewards)[-self.batch_size:])
                adv = (rewards - rewards.mean()) / (rewards.std() + 1e-10)
                adv_t = torch.FloatTensor(adv).unsqueeze(1)

                for _ in range(5):
                    pred = self._policy(states)
                    policy_loss = (pred - actions).pow(2).mean()
                    optimizer.zero_grad()
