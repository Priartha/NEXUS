...
def _psychology_adjustment(
    ...
    """Adjust signal confidence based on market psychology."""
    if not psychology:
    ...
    # 1. Fear/greed extremes are contrarian signals
    if psychology.fear_greed_label == "extreme_fear" and direction == "buy":
        adjustment += 0.08
        reasons.append("Extreme fear - contrarian buy opportunity")
    elif psychology.fear_greed_label == "extreme_greed" and direction == "sell":
        adjustment += 0.08
        reasons.append("Extreme greed - contrarian sell opportunity")
    elif psychology.fear_greed_label == "extreme_fear" and direction == "sell":
        adjustment -= 0.10
        reasons.append("Extreme fear - avoid selling into capitulation")
    elif psychology.fear_greed_label == "extreme_greed" and direction == "buy":
        adjustment -= 0.10
        reasons.append("Extreme greed - avoid buying into FOMO")

    # 2. High retail participation reduces signal quality
    if psychology.retail_participation > 0.7:
        adjustment -= 0.05
        reasons.append("High retail noise - reduced signal quality")

    # 3. Smart money activity confirms signals
    if psychology.smart_money_activity > 0.5:
        if (direction == "buy" and any(s.side == "bullish" and "smart_money" in s.type for s in psychology.active_signals)) or \
           (direction == "sell" and any(s.side == "bearish" and "smart_money" in s.type for s in psychology.active_signals)):
            adjustment += 0.06
            reasons.append("Smart money confirms direction")

    # 4. High trap risk reduces confidence
    if psychology.trap_risk > 0.7:
        adjustment -= 0.08
        reasons.append("High trap risk - false breakout likely")

    # 5. Emotional extremes
    if psychology.emotional_state == "panic" and direction == "buy":
        adjustment += 0.05
        reasons.append("Panic capitulation - potential bottom")
    elif psychology.emotional_state == "euphoric" and direction == "sell":
        adjustment += 0.05
        reasons.append("Euphoric exhaustion - potential top")
...
def _readability_adjustment(
    ...
    """Adjust signal confidence based on price action readability."""
    if not readability:
    ...
    # 1. Overall readability grade
    if readability.grade in ("A+", "A"):
        adjustment += 0.08
        reasons.append("Excellent price action clarity")
    elif readability.grade in ("B+", "B"):
        adjustment += 0.04
        reasons.append("Good price action clarity")
    elif readability.grade in ("C+", "C"):
        adjustment -= 0.02
        reasons.append("Moderate clarity - trade carefully")
    elif readability.grade in ("D", "F"):
        adjustment -= 0.10
        reasons.append("Poor readability - avoid trading")

    # 2. Noise level
    if readability.noise_level > 0.7:
        adjustment -= 0.06
        reasons.append("High noise environment")
    elif readability.noise_level < 0.3:
        adjustment += 0.04
        reasons.append("Clean price action - low noise")

    # 3. Structure reliability
    if readability.structure_reliability > 0.7:
        adjustment += 0.05
        reasons.append("Reliable market structure")
    elif readability.structure_reliability < 0.4:
        adjustment -= 0.06
        reasons.append("Unreliable structure - wait for clarity")

    # 4. Tradeability assessment
    if readability.tradeability == "avoid":
        adjustment -= 0.15
        reasons.append("Market conditions suggest avoiding trades")
    elif readability.tradeability == "excellent":
        adjustment += 0.06
        reasons.append("Excellent tradeability conditions")
...
def detect_trade_signals(
    ...
    - FIX #6: Volume profile integration
    - FIX #7: Market psychology integration
    - FIX #8: Price action readability scoring
    """
    if len(candles) < 100:
        return []
    ...
    for direction in ["buy", "sell"]:
        is_uptrend = direction == "buy" and trend_strength > 0.50
        is_downtrend = direction == "sell" and trend_strength < -0.50
