...
class BacktestEngine:
    ...
    def _run_walk_forward(
        ...
        peak = combined_equity[0]["account_balance"] if combined_equity else 0
        for e in combined_equity:
            if e["account_balance"] > peak:
                peak = e["account_balance"]
            e["drawdown_pct"] = (peak - e["account_balance"]) / peak if peak > 0 else 0
        ...
        for i in range(min_candles, len(candles)):
            ...
            for trade in list(open_trades):
                ...
                def apply_exit_costs(raw_exit_price: float) -> tuple[float, float, float, float]:
                    ...
                    funding_cost = funding_per_bar * entry * qty * bars_held
                ...
                if bars_held >= self.max_hold_bars + 1:
                    ...
                    balance += pnl
                    returns_series.append(pnl / prev_balance if prev_balance > 0 else 0)
    ...
    def _compute_stats(self, results, equity, balance, candles, symbol="", timeframe="", start_balance: float | None = None):
        ...
        returns = [e for e in returns_series if e is not None] if returns_series else []
