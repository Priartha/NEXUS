...
class UnifiedScalpEngine:
    ...
    def __init__(self) -> None:
        ...
        self._last_sl_level: Optional[float] = None
    ...
    def _previous_sl_breached(self, candles: list[Candle], now_ms: int) -> str:
        if self._last_sl_level is None or not self._last_sl_side or not self._last_sl_signal_ts:
            return ""
        if not candles:
            return ""
    ...
    def _sl_cooldown_active(self, now_ms: int) -> bool:
    ...
    def compute(
        ...
        self._last_liq_levels = self._context_value(futures_context, 'liquidation_clusters', [])
        ...
        if momentum_strong and cooldown_remaining >= self._signal_cooldown_ms and not self._sl_cooldown_active(now_ms):
        ...
        sl_blocker = ""
        if self._last_sl_side == winning_side and self._sl_cooldown_active(now_ms):
    ...
    def _build_signal(self, signal_type: str, price: float, atr: float, score: float, reasons: list[str], now_ms: int, fr: ScalpFundingRate | None = None, enriched_features: dict | None = None, candle: Candle | None = None, regime: MarketRegime | None = None, hmm_regime: HMMRegime | None = None, adaptive_sltp: dict | None = None, kelly: dict | None = None) -> ScalpSignal | None:
        ...
        try:
            ...
            if candles_for_adaptive:
                liq_levels = [lvl.get("price") if isinstance(lvl, dict) else lvl.price for lvl in getattr(self, '_last_liq_levels', [])] if hasattr(self, '_last_liq_levels') else None
    ...
    def _regime_adaptive_sltp(self, price: float, atr: float, score: float, regime: MarketRegime | None) -> dict:
        ...
        elif regime.phase == "volatile":
            result["sl_mult"] = base_sl * 1.4
            result["tp1_mult"] = base_tp1 * 1.2
            result["tp2_mult"] = base_tp2 * 1.2
            result["reason"] = f"volatile: wider stops (×1.4)"
        else:
            result["reason"] = f"{regime.phase}: standard SL/TP"
        return result
