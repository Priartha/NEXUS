"""Test: What if we only take buy signals?"""
...
def backtest_variant(name, detect_fn, candles, only_buy=False, only_sell=False, step=10):
    exits = []
    ...
    for i in range(200, len(candles) - 50, step):
        ...
        if sigs:
            ...
            # Filter by direction
            if only_buy and sig.side != "buy":
                continue
            if only_sell and sig.side != "sell":
                continue
...
async def main():
    ...
    print("=" * 80)
    print("NEXUS v3 - VARIANT ANALYSIS")
    print("=" * 80)
    ...
    print("\n[1/3] All signals...")
    start = time.time()
    r = backtest_variant("all_signals", detect_v3, candles, step=10)
    print(f"  {time.time()-start:.1f}s: {r['signals']} signals, WR: {r['win_rate']:.1f}%, PnL: ${r['total_pnl']:.2f}")
    results.append(r)
    
    print("\n[2/3] Buy signals only...")
    start = time.time()
    r = backtest_variant("buy_only", detect_v3, candles, only_buy=True, step=10)
    print(f"  {time.time()-start:.1f}s: {r['signals']} signals, WR: {r['win_rate']:.1f}%, PnL: ${r['total_pnl']:.2f}")
    results.append(r)
    
    print("\n[3/3] Sell signals only...")
    start = time.time()
    r = backtest_variant("sell_only", detect_v3, candles, only_sell=True, step=10)
    print(f"  {time.time()-start:.1f}s: {r['signals']} signals, WR: {r['win_rate']:.1f}%, PnL: ${r['total_pnl']:.2f}")
    results.append(r)
    
    print("\n" + "=" * 80)
    print(f"{'Variant':<20} {'Signals':>8} {'Win%':>7} {'PF':>6} {'AvgWin':>9} {'AvgLoss':>9} {'TotalPnL':>10}")
    print("-" * 80)
    for r in results:
